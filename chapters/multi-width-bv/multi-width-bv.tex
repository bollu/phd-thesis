...
came first, both in this thesis and in the telling---only once the mono-width
procedure was in hand did it become clear that the far more general
...
and \cref{ch:mono-width} showed how to decide predicates in the
...
\begin{enumerate}
  ...
  procedures of \cref{ch:mono-width}, a new \emph{sound and complete
...
The automata-theoretic and model-checking machinery on which our multi-width
solver rests---bitwidth-independent circuits, automata encodings of Presburger
arithmetic, and mechanized model checking---is surveyed in the related work of
\cref{ch:mono-width}. Here we discuss the work most specific to the multi-width
setting: fixed-width backends, prior parametric-bitvector solvers, and bounded
model checking.
...
\label{sec:multi:conclusion}

For the multi-width theory $\text{PBV}_n$, we establish a key structural
property: an equisatisfiable translation from $\text{PBV}_n$ to $\text{PBV}_1$,
based on the idea of encoding widths themselves as bitvector masks. This enables
two solvers. The first is a sound and complete decision procedure for the
linear-bitwise fragment of $\text{PBV}_n$ that handles operations which change
bitvector widths. The second is a bounded model checker that encodes the full
$\text{PBV}_n$ theory as a \emph{single} \qfbv{} query, avoiding the exponential
enumeration required by prior approaches. Both are implemented in Lean, with the
unbounded solver producing fully mechanized proofs via verified automata
construction and $k$-induction. Across \EverybenchNumProblemsTotal{} problems,
BMC-Ours saturates nearly every dataset with a single \qfbv{} query, solving
...
Taken together with the mono-width procedure of \cref{ch:mono-width}, the two
reductions establish a foundation for parametric bitvector reasoning, grounding
width-independent automation in mechanized proofs and establishing Lean as a
platform for trustworthy, scalable bitvector reasoning across cryptography,
hardware, and verified compilers.
...
The evaluation of this chapter is packaged into a Docker container as the
artifact for the multi-width results~\citep{bhat2026multiwidth}.
