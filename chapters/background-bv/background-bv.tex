...
but refer the reader to~\citep{kroening2016decision} for the algorithmics of SMT solving,
to \citep{hodges1997shorter} for model theory, and \citep{boolos2002computability} for my favourite introduction to logic.
Computability and logic also has an extremely slick proof of Godel's incompleteness as a corollary
...
specifies these theories. The theories relevant to this chapter is \qfbv{}, the
...
To prove a universally quantified fixed-width statement, such as $\forall x y : \bitvec{64}, x + y = y + x$,
one first negates it to get an existentially quantified statement $\exists x, y : \bitvec{64}, x + y \neq y + x$.
...
To find counterexamples, we covert the bitvector problem into a  boolean satisfiability problem by the bitblasting approach mentioned above.
One then asks a SAT solver whether the resulting SAT formula/circuit is satisfiable.
If the formula is satisfiabile, we have found a counterexample!
...
Intuitively, this is because while $SAT$ is in $NP$, which is defined by having a polynomially verifiable certificate,
$UNSAT$ is in $coNP$, which is defined by having a polynomially verifiable refutation.
This brings us to the wonderful world of UNSAT cerificates and proof complexity,
...
The de facto standard here are the DRAT~\citep{heule2016drat} and LRAT~\citep{cruz2017efficient} formats.
...
LRAT is expression enough to express all the preprocessing tricks that SAT solvers perform,
while being conicse enough to be checked efficiently.
...
by using Lean as a programming language, and is thus not an extreme assumption to make that the compiler is correct.
