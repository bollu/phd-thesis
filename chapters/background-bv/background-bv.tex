...
\sid{Pick an epigraph for this chapter.}
...
checking, Lean's bitvector library, and Lean's verified bitblaster
\bvdecide{}.
...
the empty clause (a contradiction) from the input CNF. The de facto standard
here is the DRAT format~\citep{heule2016drat}, in which each step records only
the clause derived, leaving the checker to rediscover why that step is sound.
The LRAT format~\citep{cruz2017efficient} instead annotates each derivation
step with hints identifying the clauses it depends on, which makes certificates
checkable by a simple and efficient program that is small enough to verify
formally. The
...
fast \texttt{Nat} arithmetic. The representation is
...
the \texttt{ofReduceBool} axiom. The user thus trusts the
...
\sid{Write a closing bridge paragraph: what \bvdecide{} cannot do (widths must
be concrete numerals), setting the stage for the parametric-width decision
procedures of \cref{ch:mono-width,ch:multi-width}.}
