...
This chapter closes the width axis of the thesis and opens the value axis. The
two preceding chapters made bitvector \emph{widths} symbolic while the values
stayed plain machine integers; here the widths return to being concrete and the
values become the most intricate objects compilers routinely reason about. The
same verified-bitblasting idea carries over: we mechanize the SMT-LIB theory of
floating point (\qffp{}), build circuits for its operations, prove them correct
against that theory, and extend \bvdecide{} to discharge floating-point goals
through them. The result is the first \emph{verified} floating-point
bitblaster---verified for every \qffp{} operation except round-to-integral,
remainder, and the integer conversions, whose circuits we implement and test but
do not yet prove.

Building the circuits from the mechanized theory upward, rather than porting them
wholesale, turns out to matter. It forces into the open the width assumptions
that SymFPU's unpacker and rounder leave implicit---assumptions on which those
circuits abort by assertion rather than silently miscompute---and discharging
them explicitly is what lets our circuits cover the small floating-point formats,
E4M3 and E5M2, that machine-learning quantization has made important
(\cref{sec:fp:surfaced}).
...
and it is worth being precise about why, because the same obstacle recurs across the thesis.
...
Existing automation closes none of them, so the proof scripts become dominated by arithmetic bookkeeping that is conceptually trivial but mechanically exhausting.
Effective proof automation for nonlinear reasoning, ideally operating uniformly across \texttt{Nat}, \texttt{Int}, \texttt{Rat}, and \texttt{BitVec}, is therefore the key enabling technology for verified floating-point arithmetic at scale---%
a thread we pick up in \cref{ch:future-work}, where it turns out to be the central lesson of the thesis rather than a difficulty local to this chapter.
