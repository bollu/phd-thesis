...
\paragraph{Closing the arc.}
Floating point completes the thesis's sweep across finite domains. Beginning
from the proof obligations that verified peephole rewriting leaves behind
(\cref{ch:lean-mlir}), we built mechanized decision procedures of growing
generality---one symbolic width, then many (\cref{ch:parametric-bv}), and
finally the bit-level intricacy of floating-point arithmetic---each
implemented, proved correct, and run inside Lean at a speed competitive with
the unverified state of the art. This is the sense in which decision procedures
for finite-domain theories can be mechanized without surrendering the
