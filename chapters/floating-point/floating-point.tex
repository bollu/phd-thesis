...
floating-point bitblaster---verified for every \qffp{} operation except
round-to-integral, remainder, and the integer conversions, whose circuits we
implement and test but do not yet prove---a library and associated tactic in
the Lean proof assistant that
...
solver for these formats (modulo the same three unproven operations).
...
This same nonlinear bottleneck recurs across the thesis, and we take it up as a decision problem in its own right---via the first-order theory of the $2$-adic integers---in \cref{sec:fw:2adic}.
Two further directions extend this chapter: reaching theory parity with Bitwuzla by adding incremental solving, ackermannization, and the theory of arrays (\cref{sec:fw:ackermannization}), and moving from worst-case to average-case floating-point error bounds through $\delta$-satisfiability over the reals (\cref{sec:fw:deltasat}).
