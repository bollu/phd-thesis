...
\epigraph{Floating-point arithmetic is by nature inexact, and it is not
difficult to misuse it so that the computed answers consist almost entirely of
``noise.''}{Donald E. Knuth}
...
% Links into our Lean development, in the style of rocq-sims source links.
% \linklean[ShortLabel]{Path/In/Repo.lean}{line} renders [L∃∀N ShortLabel:line]
% (defaulting to the full path) and hyperlinks to the file and line on GitHub.
% \leanlinkroot links to the repository root.
\providecolor{leanblue}{HTML}{283593}
\newcommand{\leanlogo}{{\small \ensuremath{\texttt{L}\exists\forall\texttt{N}}}~~}
\newcommand{\leanprojecturl}{https://github.com/opencompl/fp.lean/}
\newcommand{\leanrepourl}{https://github.com/opencompl/fp.lean/blob/main}
\newcommand{\leanlink}[2]{\href{#1}{{\color{leanblue}[{\footnotesize\leanlogo}\,\texttt{\footnotesize #2}]}}}
\newcommand{\leanlinkroot}{\leanlink{\leanprojecturl}{fp.lean}}
\newcommand{\linklean}[3][]{%
  \ifthenelse{\isempty{#1}}%
    {\leanlink{\leanrepourl/#2\#L#3}{#2:#3}}%
    {\leanlink{\leanrepourl/#2\#L#3}{#1:#3}}}
...
via the multiplication bitblasting circuit (summarized in \autoref{fig:fp:algo-overview}).
...
The SMT-LIB floating-point theory~\cite{smtlibfpa}
...
and it is the specification that SMT solvers such as Z3~\cite{z3},
cvc5~\cite{cvc5}, and Bitwuzla~\cite{bitwuzla} implement.
...
circuit-correctness proofs tractable (\autoref{sec:fp:operation-proofs}).
Third, it must be \emph{executable}, so that the specification itself can be run and differentially tested (\autoref{sec:fp:testing}).
...
-- The SMT-LIB semantics of multiplication.
def mul (M : RoundMethod (PackedFloat e s) R)
    (rm : RoundingMode)
    (x y : PackedFloat e s) : PackedFloat e s :=
  -- the exact product, computed in R
  let z    := M.embed x * M.embed y
  -- IEEE sign rule: XOR of the operand signs
...
the packed floats (\texttt{PackedFloat e s}) that the operation consumes and produces (\autoref{sec:fp:ieee754}),
the numeric domain \texttt{R} in which the exact product is computed (\autoref{sec:fp:domain}),
and the round method \texttt{M} that maps the exact product back into the format (\autoref{sec:fp:adjunction}).
...
where the sign is represented by a single bit and the exponent and significand are represented by unsigned integer bit-vectors.
Our \texttt{PackedFloat}\footnote{\linklean[PackedFloat/Basic.lean]{Fp/PackedFloat/Basic.lean}{20}.
Lean source links resolve to the corresponding file and line of our development, \leanlinkroot{}.} datatype
...
Writing $E$ and $M$ for the lengths of the exponent and significand bit-vectors, a \emph{normal} number denotes:
\begin{equation}
...
To model this, we define the relation \texttt{EquivUptoNaN}\footnote{\linklean[PackedFloat/Basic.lean]{Fp/PackedFloat/Basic.lean}{281}},
...
relation~\linklean[SmtLibSemantics.lean]{Fp/SmtLibSemantics.lean}{282}:
\begin{lean4}
...
force\footnote{\linklean[Enumerator.lean]{FpExecutableTests/SmtLibSemanticsEnumerator.lean}{18}}:
...
which both simplifies proofs and provides a test oracle for the circuit implementation (\autoref{sec:fp:testing}).
...
Note that we mechanize all operations except for conversions between floating-point and integer/bitvector formats and rem (\autoref{tab:fp:supported-ops}). We continue sticking with multiplication as the running example, and use other operations to draw comparisons and highlight differences when necessary.
...
\textbf{Impl.} implemented and tested (\autoref{sec:fp:testing}).
\textbf{FP8 oracle.} operation results agree with the PyMPF oracle for FP8 (\autoref{sec:fp:fptg-oracle}).}
...
\noindent This is the same structure that SymFPU uses to encode floating point into bit-vectors, but what is new is that we mechanize it and prove each stage correct against the SMT-LIB semantics.
...
Next, we describe the rounder first (\autoref{sec:fp:rounding}), because it is the most complex stage, and it fixes the contract for the operation implementation.
...
Next, we describe the precise idea behind the rounding circuit, and then the operations themselves (\autoref{sec:fp:operations}),
...
from packed inputs to packed outputs, for all operations as indicated in \autoref{tab:fp:supported-ops}.
...
Therefore, we have a predicate \texttt{Packable}\footnote{\linklean[packable.lean]{Fp/UnpackedFloat/packable.lean}{14}. We prove that \texttt{Packable} characterizes exactly the image of unpacking: a \texttt{Packable} float packs and unpacks back to itself~\linklean[PackUnpackRoundTrip.lean]{Fp/Theorems/PackUnpackRoundTrip.lean}{52}, and conversely, the unpacking of any packed float is \texttt{Packable}~\linklean[Packing.lean]{Fp/Theorems/PackedFloat/Packing.lean}{924}.} that checks whether an unpacked float can be packed back into the packed representation.
...
and the sticky bit tells us whether we are exactly at the midpoint or not (\autoref{fig:fp:guard-sticky}).
...
~\linklean[StickyEquiv.lean]{Fp/Theorems/StickyEquiv.lean}{16}
\end{definition}
...
The master lemma~\linklean[RoundEquiv.lean]{Fp/Theorems/RoundEquiv.lean}{367}
...
\texttt{UnpackedFloat.toExtRat\_round\_Rel\_smtLibRound}~\linklean[AllRoundingModes.lean]{Fp/Theorems/AllRoundingModes.lean}{30} in our development,
which dispatches on the rounding mode to five per-mode bridge lemmas (e.g.,~\texttt{\ldots\_of\_RNE}~\linklean[Round/RNE.lean]{Fp/Theorems/UnpackedFloat/Round/RNE.lean}{720}).}
\end{theorem}
...
and the combination of guard bit and sticky bit determines whether $x$ is below, above, or exactly at the midpoint (\autoref{fig:fp:guard-sticky}).
...
\texttt{Fp.mul\_eq\_mul}~\linklean[Multiplication.lean]{Fp/Theorems/Multiplication.lean}{303} in our development.
Its analogues are
\linklean[Addition.lean]{Fp/Theorems/Addition.lean}{1777},
\linklean[Division.lean]{Fp/Theorems/Division.lean}{424}, and
\linklean[FMA.lean]{Fp/Theorems/FMA.lean}{287}.}
\begin{lean4}
theorem mul_eq_mul (a b : PackedFloat e s) (rm : RoundingMode)
    (he : 3 ≤ e) (hs : 1 ≤ s) :
  (SmtLibFunctions.mul rm a b).EquivUptoNaN (PackedFloat.mul rm a b)
\end{lean4}
\noindent The left-hand side is the SMT-LIB specification (\autoref{sec:fp:spec}):
embed into the extended rationals, multiply exactly, round.
The right-hand side is the bit-level circuit (\autoref{sec:fp:operations}).
...
with this approach (\autoref{tab:fp:supported-ops}).
...
we prove that the result of the square root operation is closer to the exact result than either of the two nearest representable numbers. \sid{TODO: link code}
...
Assembling the verified circuits of \autoref{sec:fp:design} into a complete,
...
We use the verified circuits of \autoref{sec:fp:design} to build a complete floating-point solver
...
a bitblasting problem that is handed off to Lean's QF\_BV solver, \bvdecide{}~(\autoref{fig:fp:architecture}).
...
We synthesize the \texttt{Decidable} instance for this quantifier~\linklean[PackedFloat/Basic.lean]{Fp/PackedFloat/Basic.lean}{677}.
...
Each circuit is proven equal to the SMT-LIB semantics (\autoref{sec:fp:operation-proofs}), so every value the enumerator computes agrees with the specification.
...
bit-widths in the rounder that were one bit too short and patched SymFPU to prevent it
...
and the float-to-bit-vector conversion fixes \symfpucommit{40bdec0} (April 2026) and \symfpucommit{506e5e7} (May 2026).}
...
This is the output of Schanda's floating-point test generator~\cite{brain2019building},
...
\autoref{tab:fp:supported-ops} records this as the \textbf{Exhaustive FP8 oracle}
column.
...
Our theorem \texttt{div\_eq\_div}~\linklean[Division.lean]{Fp/Theorems/Division.lean}{424}
...
assertion on formats where the assumption fails (\autoref{sec:fp:testing}).
Our per-operation theorems (\autoref{sec:fp:operation-proofs}) instead carry two side
...
To evaluate our solver, we ask
whether it is competitive with the state of the art,
and how well we do on small floating-point formats,
whose unknown soundess in current solvers we want to fix due to application to ML workloads. We crystallize these into the following research questions:

\noindent\textbf{RQ1} How does our solver compare against the state-of-the-art FP solver, Bitwuzla,
and what is the overhead of our mechanized implementation against the unverified implementation?

\noindent\textbf{RQ2} How much faster is our solver than exhaustive enumeration on small floating-point problems?
In theory, since small floating-point formats such as FP8 only have $2^8$ values,
exhaustive enumeration is a feasible, if not fast, strategy.
...
\texttt{sat}, \autoref{sec:fp:frontends}),
...
In both modes, the trusted code base contains the frontend that parses SMT-LIB problems into our internal formulas (\autoref{sec:fp:frontends}),
...
via the \texttt{Decidable}-instance enumeration described in \autoref{sec:fp:frontends}.
...
\caption{\textbf{RQ1.} Our solver stays within a \SmtLibGeomeanTimeElapsedRatioOB{} geomean slowdown of
...
(discharged by reconstructing and kernel-validating a floating-point model, \autoref{sec:fp:frontends}).
...
With this, we answer \textbf{RQ1} in the affirmative:
our solver scales to standard benchmarks within a \SmtLibGeomeanTimeElapsedRatioOB{} geomean
slowdown of the state-of-the-art Bitwuzla.

\subsection{Our Solver Is Faster Than Exhaustive Enumeration for Small Floating-Point Formats}

\autoref{fig:fp:small-fp-cactus} shows the cactus plot comparing our solver
...
We answer \textbf{RQ2} affirmitively, as our solver is faster than exhaustive enumeration by a geomean factor of \SmallFpTimeElapsedRatioON{} and by \SmallFpTimeoutSpeedupLB{} on the hardest instances,
...
\caption{\textbf{RQ2.} On the small FP formats (E4M3/E5M2) from the LLVM InstCombine rewrites, our verified
...
We answer \textbf{RQ2} as follows: our solver outperforms exhaustive enumeration by
...
\paragraph*{Floating Point Bit-Blasting and Solvers}
...
to bit-vector operations, and is used internally by both cvc5~\cite{cvc5} and
Bitwuzla~\cite{bitwuzla}. SymFPU encodes each floating-point operation as a
bit-vector circuit, but the encodings are unverified and, as we discovered,
contain implicit assumptions relating the significand and exponent widths
that exclude the small formats used in machine learning (\autoref{sec:fp:testing}).
Our work mechanizes and generalizes SymFPU's algorithms, removing some of these restrictions,
and provides a machine-checked proof of correctness for the bit-blasting encodings.

\citet{brain2019building} describes the design of SymFPU's bit-blasting encodings,
...
Overall, we owe a great deal to the line of research that has developed the SMT-LIB floating-point theory and bit-blasting encodings,
and our work mechanizes and verifies these contributions by standing on their shoulders.

\section{Conclusions, Limitations, and Future Work}
\label{sec:fp:conclusion}

We create the first mechanized bitblaster for the QF\_FP theory,
and in doing so,
show that floating point SMT solving is well within the reach of formal verification.
Three operations remain unproven:
round-to-integral, remainder, and the conversions to \texttt{ubv} and \texttt{sbv}.
Their circuits are implemented and the solver bitblasts them,
but what is missing is the mechanized correctness proof.
...
the obligations that arise are overwhelmingly nonlinear, mixing multiplication of variables, integer division and modular arithmetic, powers of two with variable exponents, and bitvector operations,
and because existing automation closes none of them, the proof scripts become dominated by arithmetic bookkeeping that is conceptually trivial but mechanically exhausting.
We therefore see effective proof automation for nonlinear reasoning, ideally operating uniformly across \texttt{Nat}, \texttt{Int}, \texttt{Rat}, and \texttt{BitVec}, as the key enabling technology for verified floating-point arithmetic at scale.


Overall, we mechanize the chain from SMT-LIB semantics down to circuits and symbolic rewriting,
with each link proven correct relative to the QF\_FP theory.
Along the way, we discover and repair implicit assumptions in SymFPU's circuits,
generalizing them to support formats (e.g.\ E4M3, E5M2) that are increasingly important for emerging domains such as machine learning.
We exercise our mechanized semantics against independent reference implementations,
thereby validating that our mechanization faithfully implements the SMT-LIB floating-point specification
and providing trustworthiness in our implementation.
Our evaluation shows that verified bitblasting is practical:
on SMT-COMP, a stratified cross-family sample of the SMT-LIB floating-point benchmarks,
our solver certifies \SmtLibNumSolvedOurs{} of \SmtLibNumTotal{} instances within a
\SmtLibGeomeanTimeElapsedRatioOB{} geomean slowdown of Bitwuzla (and never disagrees with a known status),
in exchange for end-to-end correctness guarantees,
while on small floating-point formats our solver is \SmallFpTimeElapsedRatioOKernel{} faster than the sound,
exhaustive-enumeration baseline, and \SmallFpTimeoutSpeedupLB{} faster on the hardest instances that enumeration times out on.
Thus, we close the gap between verified reasoning and practical floating-point SMT solving.
