#!/usr/bin/env python3
# Plot programs with lowest bitwidth that timeout
...
def setGlobalDefaults():
    ...
    # inches
    # A4 size sheet is 8.3 (width) * 11.7 (height) inches
    WIDTH = 8.3
    HEIGHT = 11.7
    matplotlib.rcParams['figure.figsize'] = WIDTH * 0.85, HEIGHT // 3
...
def plot_slowest_bitwidths():
    ...
    bws = list(bws); bws.sort()
    TIMEOUT = 3600
    # 2) for each bitwidth from smallest to largest, find programs that timed out in TIMEOUT seconds
    # 3) for a fixed timeout (1h), make a plot of program v/s bitwidth necessary to timeout
    plotrows = []
    for cur_bw in bws:
        with open("alive.csv", "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                bw = int(math.log2(int(row["bitwidth"])))
                if row["name"] == "Select:1087": continue # absurdly large
                if bw == cur_bw and int(row["timeout"]) == 3600 and row["did_timeout"] == "True":
                    plotrows.append(row)
                    print(row)
    plotrows.sort(key = lambda val: int(val["bitwidth"])) # sort by bitwidth

    fig, ax = plt.subplots()
    ax.set_axisbelow(True)
    ax.yaxis.grid(linewidth=.2, linestyle='dotted', color=black)
    def get_name(row):
        ...
        if name.isdigit():
                return path + name
        return name
    names = np.array([get_name(row) for row in plotrows])
    bitwidths = np.array([int(row["bitwidth"]) for row in plotrows])
    xs = np.arange(len(plotrows))  # the label locations
    ...
    for i, name in enumerate(names):
        CUTOFF = 30
        if len(name) > CUTOFF: names[i] = name[:CUTOFF-3] + "..."

    ax.set_ylabel('Bitwidth for 1hr verification timeout',
                  rotation='horizontal',
                  position=(1, 1.05),
                  horizontalalignment='left',
                  verticalalignment='bottom')
    ...
    # autolabel(ax, rects1)
    save(fig, "lowest-bitwidth-timeout.pdf")
