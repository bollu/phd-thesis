...
This is where the thesis begins: before we can automate the finite-domain
obligations that rewrites leave behind, we need a framework in which the
rewrites themselves can be stated, composed, and proved correct. In this
chapter we combine the convenience of automation with the versatility
...
We show how to model control flow in \cref{sec:mlir:structured-control-flow}.
...
The ``Poly'' IR is, in fact, a superset of the \texttt{QuotRing} IR we defined in \cref{sec:mlir:motivation}.
It consists of the operations \texttt{add}, \texttt{sub}, \texttt{mul}, \texttt{mul\_constant}, \texttt{leading\_term}, \texttt{monomial}, \texttt{monomial\_mul}, \texttt{from\_tensor}, \texttt{to\_tensor}, \texttt{arith.constant} and \texttt{constant}.\footnote{It also has distinct types for integers and naturals, which we unified in \cref{sec:mlir:motivation} for simplicity.}
...
Several limitations bound what we prove, and each points to concrete future
work. First, we model side effects coarsely: rewriting is proved sound only for
\texttt{EffectKind.pure} operations, and the one genuinely side-effectful
behaviour we encounter---the immediate undefined behaviour that division and
remainder may raise---is collapsed together with poison into
\texttt{Option.none} rather than tracked as a first-class effect. This is sound
for the rewrites we translate, because we check that either side triggers
\texttt{ub} exactly when the other does; but separating \texttt{ub} from poison
as a distinct side effect, and reasoning about rewrites that refine it, remains
future work. Second, because our semantics denote into total Lean functions,
the user-supplied semantics of a region must be provably terminating; richer
denotations---computable coinductive and noncomputable domain-theoretic
semantics---are a natural next step for modelling non-terminating and
higher-order regions.

The case studies of \cref{sec:mlir:casestudies} expose the third, and central,
limitation, which is not one of the framework but of the automation beneath it.
Once \texttt{simp\_peephole} has stripped away the SSA scaffolding, each rewrite
leaves behind a proof obligation over bitvectors of arbitrary width, and only
$54$ of the $93$ translated Alive rewrites could be discharged by the automation
available at the time. The framework discharges the structural part of every one
of these rewrites; the $39$ that remain fail not because \messa{X} cannot state
or compose them, but because the residual bitvector goals outrun the general-%
purpose tactics then available. Closing this automation gap---deciding such
bitvector goals at scale, rather than proving them by hand---is precisely the
subject of \cref{ch:mono-width,ch:multi-width}, and the reason the rest of the thesis is
devoted to verified decision procedures.

\paragraph{Data availability.}
The \messa{X} framework is developed openly on
GitHub~\cite{leanmlirgithub}, and the artifact accompanying the ITP~2024
publication is archived on Zenodo at
