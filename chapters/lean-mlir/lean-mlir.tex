...
\lettrine{I}{n} our aim to verify compiler optimizations,
we begin by focusing on the \emph{peephole rewriting} of SSA-based intermediate representations (IRs).
Static single assignment (SSA)~\citep{rastello2022ssa}
intermediate representations are at the core of modern compilers, 
thanks to their encoding of use-def relationships,
which makes it incredibly easy to implement various compiler analyses and transformations.
In particular, \emph{Peephole optimizations}~\citep{mckeeman1965peephole}, which replace instruction
sequences of bounded length with semantically equivalent optimized ones,
are made much more effective by use of SSA, since it allows a compiler to reason
about expression trees of variables, rather than just linear sequences of instructions.
Such optimizations are so ubiquitous that 10\% of all IR-transforming code in
LLVM~\citep{lattner2004llvm} belongs to its InstCombine peephole optimizer.
Note that this is much more than the size of LLVM's loop optimizer!
The most referenced tool in LLVM's commit log is the Alive peephole verifier~\citep{lopes2021alive2},
which brought automatic SMT-based verification into the day-to-day of the LLVM community.
...
covers \emph{regions}, which are the nested scoping used by many domain-specific IRs in
the MLIR ecosystem. Moreover, we mechanize these results in Lean,
and create a framework that enables us to perform peephole rewrite verification,
as our framework is coupled with a lifting theorem, that lifts the local correctness of a peephole rewrite
into a global correctness theorem for the entire program transformation.
Overall, this builds the foundation for a verified peephole rewriter,
and reduces the problem to one where we must then reason about the semantics of the IR itself.
This then naturally leads its way into decision procedures for proving equivalence of bitvector and floating point programs.
...
As an example of our framework, we model an IR aimed at FHE that manipulates objects in the algebraic structure $R \equiv (\mathbb{Z}/q\mathbb{Z})[X] / (X^{2^n} + 1)$.
...
This is unsupported, as peephole rewrites in \messa{X} may only have free variables, not free region arguments.
...
See that the parametricity of our framework allows us to prove theorems that are valid on all IR extensions $\texttt{scf}(X)$.
...
behaviour we encounter, which is the immediate undefined behaviour of division and remainder may raise,
is collapsed together with poison, rather than being tracker as a first class effect.

The case studies of \cref{sec:mlir:casestudies} exposes a limitation of the automation.
...
available at the time. Closing this automation gap, and being able to decide such bitvector goals at scale,
rather than proving them by hand, is precisely the subject we tackle next,
and the reason the rest of the thesis is devoted to verified decision procedures.
