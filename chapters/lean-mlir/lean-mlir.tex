...
IR-transforming code---much more than its loop optimizer!---and the Alive
verifier~\citep{lopes2021alive2} made SMT-based validation of such rewrites
part of the LLVM community's daily routine. How, then, should we verify a
peephole rewriter? The two answers on offer each give up something we need.
CompCert-style end-to-end verification fixes the language up front, and so
cannot keep pace with IRs that are still evolving---which is precisely the
situation in the MLIR ecosystem~\citep{lattner2021mlir}, where every domain
grows an IR of its own. Alive-style translation validation is push-button, but
it is bound to one fixed IR and rests on an unverified toolchain. This chapter
takes a third route, combining the convenience of Alive-style automation with
machine-checked proof in an interactive theorem prover. I formalize a core
calculus for SSA-based IRs that is generic over the IR and covers
\emph{regions}, the nested scoping from which MLIR-style IRs are built, and
mechanize it in Lean. The central result is a lifting theorem: a rewrite proved
correct in isolation is correct wherever the rewriter fires it inside a whole
program.

Around this theorem I build a framework in which a user defines an IR by giving
three typeclass instances---the signature of each operation, and the
denotations of the IR's types and operations into Lean---and receives a
verified peephole rewriter in return. The encoding is intrinsically well-typed,
so a rewritten program is well-typed by construction; the same machinery proves
dead code elimination and common subexpression elimination correct; rewriting
stays sound when islands of pure operations sit inside a side-effectful IR; and
a proof automation tactic strips the framework away, leaving the proof engineer
a goal stated purely in the semantics of the IR. Three case studies stress the
framework: LLVM's integer arithmetic, structured control flow, and an IR for
fully homomorphic encryption whose values inhabit a quotient ring that is out
of practical reach for today's SMT solvers. The framework core is roughly 2.2k
lines of Lean and the case studies together roughly 5.6k. The first case study
is also where the boundary of this chapter shows: once the framework overhead
is stripped away, each of the 93 Alive rewrites we translate leaves behind a
proof obligation over bitvectors of universally quantified width, and only 54
of them close automatically. The structural half of verified peephole rewriting
is discharged here; closing that automation gap is the semantic half, and it is
what the rest of this thesis is about.
...
refinement holds in both directions. The exposition up to
\cref{sec:mlir:effects} uses equality throughout, purely to keep the development
readable; nothing in the framework depends on that choice.
...
The rewriter is now complete, so let us summarize the relationship between the
calculus of \cref{sec:mlir:framework} and the rewriter we have actually built:
\begin{itemize}
  \item The denotation of a program descends into regions, but the
    \emph{matcher} does not. When \texttt{rewritePeephole} compares an operation
    in the pattern against one in the target, it compares their region arguments
    for syntactic equality and stops there. A rewrite whose correctness depends
    on what a region computes will therefore fail to fire, though it will never
    fire wrongly.
  \item A peephole rewrite may quantify over free variables but not over free
    \emph{region} arguments. This is why the loop transformations of
    \cref{sec:mlir:structured-control-flow}---dead loop deletion, loop fusion,
    loop reversal---are proved as theorems about the denotation of
    \messa{\texttt{scf}}, yet are not peephole rewrites that our rewriter can
    apply. Only a loop with a statically known body, such as the conversion of
    iterated addition into multiplication, falls back inside the fragment.
  \item Regions denote bounded computations only. Lean offered no reasoning
    principle for non-well-founded recursion when this framework was built, so
    \texttt{scf.for} is a bounded iteration and unbounded loops are out of
    scope. This is a limit of the instantiation rather than of the calculus: the
    proof strategy never inspects the denotation, and Lean's subsequent support
    for domain-theoretic reasoning would lift the restriction.
  \item \texttt{rewritePeephole} takes a \texttt{fuel} argument bounding how many
    rewrites it attempts. We prove that whatever it does preserves semantics; we
    do not prove that it reaches a fixed point, nor that any particular rewrite
    is ever applied. Correctness here is a safety property, not a completeness
    one.
\end{itemize}
With the rewriter and its boundaries in view, we can say precisely how it
differs from previous approaches, before turning to the evidence that it works.

\section{Related Work: Verified Compilation and Rewriting}
\label{sec:mlir:related}

The broad landscape of compiler verification was surveyed in the background.
Here we compare against the technical details of related work.

In contrast to the Alive family of
tools~\citep{lopes2015provably,lopes2021alive2,lee2019aliveinlean}, our work
builds a framework for describing full programs (rather than rewrite
snippets), and formally defines and proves the correctness of peephole
rewriting within a larger program context.
The semantics and correctness of compiling compositionally have been explored by multiple authors, like Pilsner~\cite{neis2015pilsner} or many variants of CompCert~\cite{leroy2016compcert}:
...
This addresses an important aspect we don't model yet: what occurs at the boundaries of IRs, when mixing them.
...
Having placed the framework against its neighbours, we now put it to work.

\section{Case Studies}
\label{sec:mlir:casestudies}

A framework generic over the IR is only worth its complexity if it is genuinely
generic, and only worth using if instantiating it is cheaper than starting over.
We mechanize three IRs drawn from the MLIR ecosystem to ask exactly that:
\begin{description}
  \item[RQ1 (Generality).] Does one framework, fixed in advance, carry IRs that
    make incompatible demands of it---a low-level IR needing bitvector reasoning
    at every width, an IR needing control flow it was never designed for, and an
    IR whose values are cosets of an ideal?
  \item[RQ2 (Cost).] What does instantiating the framework cost, and where does
    the residual cost sit once it is instantiated?
\end{description}
Each case study closes out part of RQ1: the three are chosen because a framework
could plausibly succeed at any one of them by accident and fail the others.
Bitvectors (\cref{sec:mlir:bitvectors}) test whether we can match a mature
push-button tool on its home ground; structured control flow
(\cref{sec:mlir:structured-control-flow}) tests whether the framework's
parametricity survives contact with a feature added after the fact; and fully
homomorphic encryption (\cref{sec:mlir:fully-homomorphic-encryption}) tests
whether we can reach a domain no SMT solver can be pointed at. RQ2 is settled
once all three are in hand (\cref{sec:mlir:cost}), and its answer is what
carries the thesis into the chapters that follow.
...
\paragraph{RQ1: LLVM rewrites proved at every width.}
The framework meets a mature push-button tool on its home ground and, on the
fragment we model, improves on it in the way that matters for a compiler: an
Alive proof holds at the width it was run at, whereas ours holds at all of them
at once. The qualification is that we model a fragment---no \texttt{nsw} or
\texttt{nuw} flags, no preconditions---and that $54$ of $93$ is where the
automation of the day stopped. That shortfall is not evidence against
generality; it is evidence about cost, and RQ2 is where we account for it.
Next, we show how the framework supports control flow reasoning via regions.
...
\paragraph{RQ1: control flow added without reproving the metatheory.}
Control flow was not designed into any of the IRs we had; it was added
afterwards, to an arbitrary user IR $X$, by defining \messa{\texttt{scf}(X)} as
a construction \emph{over} $X$ rather than a modification of it. Nothing in the
core calculus changed, no metatheory was reproved, and the loop theorems we get
hold uniformly for every extension $\texttt{scf}(X)$ rather than for one IR at a
time. The qualification is the one recorded earlier: because these theorems
quantify over the loop body, and a loop body is a region, they are theorems the
user applies rather than rewrites the rewriter fires. Parametricity delivered
the semantics and the proofs; it did not extend the peephole fragment.
...
\paragraph{RQ1: quotient-ring rewrites, out of reach for SMT.}
This is the case that a bitvector-shaped framework could not have absorbed by
accident. The values are cosets of an ideal, the easy rewrites are ring
identities discharged by \texttt{mathlib}'s commutative algebra, and the hard
ones (\texttt{from\_tensor(to\_tensor(p))} $\rightarrow$ \texttt{p}, or
\texttt{leading\_term}) turn on a non-canonical choice of coset representative
that has no first-order encoding to hand an SMT solver. We prove both kinds.
Taking the three studies together, RQ1 is answered affirmatively: the same
calculus, unchanged, carried a low-level IR reasoned about at all widths, an
extension adding control flow to an arbitrary IR, and an IR living in a quotient
ring---and the framework was fixed before any of them was written.

\subsection{What Instantiation Costs, and Where the Cost Moves}
\label{sec:mlir:cost}

RQ2 asks what instantiating \messa{X} costs, and where the cost goes afterwards.

The first half is answered by counting. The core of the framework---the
definitions of \texttt{Expr}, \texttt{Com}, and \texttt{PeepholeRewrite}, the
lemmas about them, and the peephole rewriting theorem---is $\approx 2.2k$ LoC,
written once. Against that, the three case studies together come to $\approx
5.6k$ LoC, and the two whole-program optimizations cost little: DCE is
$\approx 400$ LoC (\cref{sec:mlir:dce}), CSE comparably. The framework is thus
substantially smaller than what has been built on it, which is the shape one
wants: the fixed cost is paid once, and each new IR pays only for its own syntax
and semantics. A user supplies three typeclass instances and a denotation, and
inherits a verified rewriter, DCE, CSE, an MLIR parser, and the automation that
strips all of it back out of the goal.

The second half is the more consequential finding, and it is not a happy one.
Once instantiation is paid for, essentially none of the remaining cost is
structural. \texttt{simp\_peephole} reduces a rewrite to a statement in the
user's own semantics, and what is then left is a mathematical obligation that
the framework can neither help with nor get out of the way of. For
\messa{\operatorname{QuotRing}} that obligation is a ring identity and
\texttt{mathlib} closes it. For \messa{\operatorname{LLVM}} it is a bitvector
identity at universally quantified width, and there the automation of the day
closed $54$ of $93$; the other $39$ were left to hand proof, and we completed
only $6$ of them, chosen for being the ones an SMT solver struggles with even at
a single fixed width. The cost of verified peephole rewriting, in other words,
has moved. It is no longer in defining IRs or in proving the rewriter correct.
It is entirely in deciding the semantic obligations that fall out the far end,
and for the domain that matters most to a compiler---bitvectors at every
width---no adequate decision procedure existed.

\section{Discussion}
\label{sec:mlir:discussion}

We set out to verify peephole rewriting without fixing the IR in advance, and
what we have is a core calculus for SSA-based IRs, generic over the IR and
covering regions, with a mechanized lifting theorem: a rewrite proved correct in
isolation stays correct wherever the rewriter fires it inside a whole program.
The same encoding carries dead code elimination and common subexpression
elimination, and survives the move from a purely functional setting to one where
islands of pure operations sit inside a side-effectful IR. Around it sits enough
engineering---an MLIR parser, elaborators into intrinsically well-typed terms,
and the \texttt{simp\_peephole} automation---that a user writes rewrites in MLIR
syntax and is handed back a goal stated in the semantics of their own IR, with
no trace of the framework in it.

What the case studies show is narrower than what the theorems say, and the
difference is worth stating plainly. The correctness results are about
denotations, so a rewrite proved here is proved against \emph{our} model of the
IR; for LLVM, the fidelity of that model to the real thing rests on exhaustive
testing up to bitwidth $8$, and on collapsing immediate undefined behaviour
together with poison into a single \texttt{Option.none}, which is sound only
under a side condition we check by hand for each division and remainder rewrite.
Separating undefined behaviour from poison, and reasoning about rewrites that
refine one into the other, remains open.

That caveat aside, the answer to RQ2 is what carries this thesis forward. The
cost of verified peephole rewriting has not gone away; it has moved, out of the
framework and into the semantic obligations the framework hands back. The
structural half of the problem is discharged by this chapter. The other half is
a question about \emph{deciding} bitvector goals at universally quantified
width, rather than proving them one by one, and that is the subject of the rest
of this thesis.
