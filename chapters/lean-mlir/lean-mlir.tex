...
\begin{itemize}
  ...
  \item \texttt{rewritePeephole} takes a \texttt{fuel} argument bounding how many
    ...
    one.
\end{itemize}
With the rewriter and its boundaries in view, we turn to the evidence that it
works.

\section{Case Studies}
\label{sec:mlir:casestudies}

A framework for verified rewriting is worth its complexity only if it can be
pointed at the IRs people actually write, and only if rewriting in them is
pleasant enough to do. We mechanize three IRs drawn from the MLIR ecosystem to
ask exactly that:
\begin{description}
  \item[RQ1 (Semantics).] Can we give formal semantics, inside \messa{X}, to
    rich IRs as they exist in the MLIR ecosystem---not model languages built to
    be easy to formalize, but IRs with poison values, with nested regions, and
    with values drawn from nontrivial algebraic structures?
  \item[RQ2 (Rewriting).] Once such an IR is defined, can we conveniently
    perform peephole rewriting in it: state rewrites in the syntax a compiler
    engineer already uses, and be left with a proof obligation in the IR's own
    semantics rather than in the framework's encoding?
\end{description}
Each case study answers both. \messa{\operatorname{LLVM}}
(\cref{sec:mlir:bitvectors}) is the arithmetic fragment of a production IR,
poison and all, against which we replay Alive's test suite.
\messa{\texttt{scf}} (\cref{sec:mlir:structured-control-flow}) is structured
control flow, the feature that most distinguishes MLIR-style IRs from LLVM, and
the one that exercises regions. The Poly IR
(\cref{sec:mlir:fully-homomorphic-encryption}) is a real and still-evolving MLIR
IR for homomorphic encryption whose values are cosets of an ideal. Together they
span what MLIR IRs are made of: machine words, control flow, and mathematics.
Where the answer to RQ2 runs out is recorded separately (\cref{sec:mlir:cost}),
and it is what carries this thesis into the chapters that follow.
...
\paragraph{What LLVM shows: poison-aware semantics, and rewrites proved at every width.}
On RQ1, we give semantics to the arithmetic fragment of a production IR rather
than a model language: sixteen scalar operators, all \texttt{icmp} flags, and
LLVM's poison values, with the semantics cross-checked against \texttt{opt} by
exhaustive enumeration. We do not model everything---the \texttt{nsw} and
\texttt{nuw} flags are absent, and immediate undefined behaviour is collapsed
together with poison---but what we model is the real thing.
On RQ2, rewrites are stated in the MLIR syntax an LLVM engineer already writes,
and \texttt{simp\_peephole} hands back a goal about \texttt{BitVec w} with no
trace of SSA in it. What we then get from that goal is better than what Alive
gets: an Alive proof holds at the width it was run at, ours holds at every width
at once. What we do not get is a proof without effort, and
\cref{sec:mlir:cost} is where we account for that.
...
\paragraph{What \texttt{scf} shows: region semantics come cheaply, region rewrites do not.}
On RQ1, this is the feature that most separates MLIR-style IRs from LLVM, and
regions carry it: \texttt{if} and bounded \texttt{for} get denotations through
\texttt{mathlib}'s theory of iterated functions, and because
\messa{\texttt{scf}(X)} is a construction \emph{over} an arbitrary user IR $X$
rather than a modification of it, the semantics and the loop theorems hold
uniformly for every $X$. Nothing in the core calculus changed and no metatheory
was reproved.
On RQ2, this is the one case where the answer is partly negative, and it is
worth being clear about. The loop theorems quantify over the loop body, a loop
body is a region, and our rewrites may not quantify over regions; so dead loop
deletion, fusion, and reversal are theorems a user applies by hand, not rewrites
the rewriter fires. Only loops with a statically known body, such as iterated
addition becoming multiplication, fall inside the peephole fragment.
Parametricity bought us the semantics; it did not widen what can be rewritten.
...
\paragraph{What Poly shows: mathematical semantics, and rewrites \texttt{mathlib} closes.}
On RQ1, this is an IR we did not design and could not simplify: a live MLIR IR
whose values are cosets of an ideal, whose operations include three that depend
on a non-canonical choice of coset representative, and which was in flux while
we formalized it. Its semantics are written directly against \texttt{mathlib}'s
commutative algebra, which is what makes them expressible at all.
On RQ2, both kinds of rewrite go through: the ring identities such as
\texttt{mul(p,q)} $\rightarrow$ \texttt{mul(q,p)} fall out of commutativity,
and the representative-dependent ones such as
\texttt{from\_tensor(to\_tensor(p))} $\rightarrow$ \texttt{p} take real but
finite work. This is the clearest evidence for RQ2's premise, because here the
proof obligation \texttt{simp\_peephole} leaves behind is one no SMT solver
could have been handed at all; the convenience comes entirely from landing in a
proof assistant's library.

Taking the three together settles RQ1: the same calculus, fixed before any of
them was written, gave semantics to a production IR with poison, to structured
control flow over an arbitrary IR, and to an evolving IR over a quotient ring.
RQ2 is answered too, but with a boundary, and the boundary is the subject of the
next section.

\subsection{What Rewriting Costs, and Where Convenience Stops}
\label{sec:mlir:cost}

RQ2 asks whether rewriting in these IRs is convenient. Two things bear on that:
what it costs to instantiate the framework at all, and what is left to do once
it is instantiated.

The first is answered by counting. The core of the framework---the
...
The second is the more consequential finding, and it is not a happy one.
...
Placing this against prior work sharpens both claims. The broad landscape of
compiler verification was surveyed in the background; here we compare against
...
Read together, the comparison and the case studies point the same way. Against
Alive we gain generality over IRs and proofs that hold at every width; against
CompCert-style verification we gain the ability to track an IR that is still
changing. What we do not gain from any of them is a way to discharge the
obligation that is left at the end. The cost of verified peephole rewriting has
not gone away; it has moved, out of the framework and into the semantics the
framework hands back. Alive's answer to that obligation is an SMT solver, which
is fast, unverified, and fixed to a single width. Our answer has to be different
on all three counts, and building it is what the next chapter begins: we take up
Lean's verified bitblaster, \bvdecide{}, which decides bitvector goals by
reduction to SAT with a certificate the kernel rechecks. That machinery settles
the fixed-width case and gives us the substrate; the width-generic case, which
is what a compiler actually needs, is what the chapters after it are for.
