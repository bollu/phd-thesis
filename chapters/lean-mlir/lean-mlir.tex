...
\caption{\label{fig:mlir:zipper}Splitting a \texttt{Com} into a zipper at the
...
We mechanize three IRs drawn from the MLIR ecosystem. They are chosen to
investigate the following questions:
\begin{enumerate}
  \item Can \messa{X} reproduce the reasoning that Alive already automates for
    LLVM's integer arithmetic, and can it then go beyond Alive, to rewrites
    proved once for \emph{every} bitwidth rather than one width at a time?
  \item Does the parametricity of the framework pay? Concretely, can control
    flow be added to an IR that was not designed for it, without reproving the
    metatheory?
  \item Can we reach a domain that is out of practical reach for SMT solvers
    altogether, where the values a program manipulates live in a quotient ring?
  \item Is the framework economical to build on, in the sense that a user's IR
    and its optimizations cost far less than the framework itself did?
\end{enumerate}
The bitvector case study (\cref{sec:mlir:bitvectors}) answers (1), structured
control flow (\cref{sec:mlir:structured-control-flow}) answers (2), and fully
homomorphic encryption (\cref{sec:mlir:fully-homomorphic-encryption}) answers
(3). Question (4) is answered by all three together, and by a line count: the
core of our framework---the definitions of \texttt{Expr}, \texttt{Com}, and
\texttt{PeepholeRewrite}, the lemmas about them, and the peephole rewriting
theorem---is $\approx 2.2k$ LoC, against which the three case studies together
come to $\approx 5.6k$ LoC, and DCE costs $\approx 400$ LoC
(\cref{sec:mlir:dce}). The case studies are thus substantially larger than the
framework they exercise, which is the evidence we can offer that it scales to
realistic verification work.
...
This answers the first of our questions in both directions. We do reproduce
Alive-style reasoning, and our proofs hold at arbitrary universally quantified
bitwidth rather than one width at a time, save for theorems that are only true
at a particular width.\footnote{e.g., \texttt{a + b = a xor b} is true only at bitwidth 1.}
But $54$ out of $93$ is also the honest measure of how far the automation of the
day could carry us, and it is the number that motivates the remainder of this
thesis. As an example, let us consider the following rewrites:
...
This settles the third question. A domain whose values are cosets of an ideal,
where the interesting rewrites are ring identities and the uninteresting ones
depend on a non-canonical choice of coset representative, is not a domain an
SMT solver can be pointed at; it is a domain where the ambient library of a
proof assistant does the work, and \messa{X} is what connects that library to
an IR.
...
refine one into the other, remains open. Against that, the framework did carry
three genuinely different IRs, at a cost in Lean code that is dominated by the
case studies rather than by the framework itself.

The sharpest limit, though, is not structural but automatic. Once
\texttt{simp\_peephole} has stripped the SSA scaffolding away, what is left is a
proof obligation over bitvectors of universally quantified width, and only $54$
of the $93$ translated Alive rewrites could be discharged by the automation
available to us. The structural half of verified peephole rewriting is
discharged by this chapter; the semantic half is a question about deciding
bitvector goals at scale, rather than proving them by hand. That is the subject
