...
\lettrine{I}{n} our aim to verify compiler optimizations, we begin by
focusing on the \emph{peephole rewriting} of SSA-based intermediate
representations (IRs)---the workhorse transformation of modern compilers,
whose vocabulary we fixed in the background (\cref{sec:bg:ssa}). Recall that
LLVM's InstCombine peephole optimizer alone accounts for roughly 10\% of the
compiler's IR-transforming code---much more than its loop optimizer!---and
that the Alive verifier~\citep{lopes2021alive2} made SMT-based validation of
such rewrites part of the LLVM community's daily routine. There is, at the
same time, an increasing need for \emph{domain-specific} reasoning in modern
compilers, fuelling tailored SSA-based IRs as popularized by the MLIR compiler
framework~\citep{lattner2021mlir}, and neither end of the verification
spectrum surveyed earlier (\cref{sec:bg:compiler-verification}) serves these
IRs well: CompCert-style end-to-end verification cannot keep pace with IRs
that are still evolving, while Alive-style validation is push-button but
bound to one fixed IR and an unverified toolchain.
...
The broad landscape of compiler verification---CompCert and CakeML,
translation validation, and the Alive line of SMT-based peephole
verifiers---was surveyed in the background
(\cref{sec:bg:compiler-verification}); here we compare against the technical
antecedents specific to our framework.
