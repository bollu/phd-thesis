...
part of the LLVM community's daily routine. How, then, should we formally verify a
peephole rewriter? 
CompCert-style end-to-end verification fixes the language up front, and so
cannot keep pace with IRs that are still evolving.
Alive-style translation validation is push-button, but it is bound to one fixed IR and trusts unverified SMT solvers.
This chapter takes a third route, combining the convenience of Alive-style automation with
...
correct in isolation is correct wherever rewrite is applied inside a whole
program.

Around this theorem, I build a framework in which a user defines an IR and its rewrites,
and receives a verified peephole rewriter in return.
The encoding is intrinsically well-typed, so a rewritten program is well-typed by construction.
Proof automation is built that strips the framework away, leaving the proof engineer
a goal stated purely in the semantics of the IR.
We demonstrate the framework with three case studies:
LLVM's integer arithmetic, structured control flow, and an IR for
fully homomorphic encryption whose values inhabit a quotient ring that is out
of practical reach for today's SMT solvers. 
Overall, once the framework overhead is stripped away,
each of the 93 bitvector rewrites leaves behind a
...
is discharged in this chapter.
The automation gap is closed in the rest of the thesis.
