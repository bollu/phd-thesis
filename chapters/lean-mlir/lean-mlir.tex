...
the MLIR ecosystem---and mechanize it in Lean, \messa{X}, with a user-friendly
frontend that translates MLIR syntax into our calculus.

The framework's purpose within the thesis is to pay the structural proof burden
once. Once it has done so, what remains of every rewrite is an obligation over
finite-domain values, and it is those obligations---not the SSA plumbing around
them---that the remaining chapters attack.
...
\Cref{sec:intro:trust} placed this thesis among the approaches that bridge
automated speed and kernel-checked trust; here we compare against the technical
antecedents specific to verified compilation and rewriting.
