...
since the IRs that are high-level and permit good equational reaasoning are typically structured into regions.
...
% \section{Limitations} \label{sec:mlir:limitations} Several limitations bound what we prove, and each points to concrete future work. First, we model side effects coarsely: rewriting is proved sound only for \texttt{EffectKind.pure} operations, and the one genuinely side-effectful behaviour we encounter, which is the immediate undefined behaviour of division and remainder may raise, is collapsed together with poison, rather than being tracker as a first class effect. The case studies of \cref{sec:mlir:casestudies} exposes a limitation of the automation. Once \texttt{simp\_peephole} has stripped away the SSA scaffolding, each rewrite leaves behind a proof obligation over bitvectors of arbitrary width, and only $54$ of the $93$ translated Alive rewrites could be discharged by the automation available at the time. Closing this automation gap, and being able to decide such bitvector goals at scale, rather than proving them by hand, is precisely the subject we tackle next, and the reason the rest of the thesis is devoted to verified decision procedures.
