...
correctness proofs, and pay for foundational trust in solver speed rather than in the size of the trusted base.
...
FpSan~\citep{openai2026fpsan} gives floating-point level equivalence, and guarantees equivalence upto an exponential ring,
which is what ML practitioners care about. It can be implemented as a bitvector solver, and I have a prototypical implementation of it in Lean.
TensorRight~\citep{arora2025tensorright} is an algorithm for shape level equivalence,
by computing a safety cutoff on the number of dimensions that must be checked, and then checking upto that cutoff.
This is a bounded-to-unbounded transfer principle, with a very similar flavour to that ouf our automata-based solvers.
Linear layouts~\citep{zhou2026linear} encode layouts efficiently in terms of bitpatterns,
under the assumption that the layouts are all going to be powers of 2. This provides convient encodings of tensor shapes,
and makes them amenable to bitblasting. The three ingredients are therefore in place, and the only missing piece is a way to combine them.
...
Looking back, it's worth asking why bitblasting works in practice.
...
A large body of an optimizing compiler rests on peephole rewriting,
