...
\begin{figure}[tb]
  ...
  \begin{minipage}[t]{.5\textwidth}
    \begin{align*}
      \mathsf{F} \eqdef~& \mathsf{E}_1 \otimes \mathsf{E}_2 \\
      &\quad (\otimes \in \{<_u, \leq_u, \leq_s, <_s, =, \neq \}) \\
...
&\denote{\Var(x)}_\rho(i) \equiv
  ...
  \end{cases} \\
&\denote{\ofNat(i)}_\rho(i) \equiv
  \begin{cases}
  0 & i = 0 \\
  \lfloor v / 2^i \rfloor \mod 2  & \text{otherwise}
  \end{cases} \\
&\denote{s || t}_\rho(i) \equiv \denote{s}_\rho(i) || \denote{t}_\rho(i) \\
&\denote{\lnot s}_\rho(i) \equiv
  \begin{cases}
    0 & i = 0 \\
    1 - \denote{s}_\rho(i) & \text{otherwise}
  \end{cases}  \\
&\denote{P}_\rho(i) = 1 \iff \\
&\quad \text{Predicate $P$ holds at bitwidth $i$ for environment $\rho$} \\
&\denote{s = t}_\rho(i) \equiv \forall j \leq i, \denote{s}_\rho(j) = \denote{t}_\rho(j) \\
&\denote{P \land Q}_\rho(i) \equiv \denote{P}_\rho(i) \land \denote{Q}_\rho(i) \\
\end{align*}
