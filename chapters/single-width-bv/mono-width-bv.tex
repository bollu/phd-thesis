...
\label{ch:mono-width}
\epigraph{\ldots{}the different branches of Arithmetic---Ambition, Distraction,
Uglification, and Derision.}{The Mock Turtle, \emph{Alice's Adventures in
Wonderland}}
...
identities---must hold for \emph{all} widths at once. This chapter takes the
first step toward deciding such \emph{parametric} bitvector goals: it decides
predicates over a \emph{single} symbolic width---the \emph{mono-width} fragment,
$\text{PBV}_1$---by casting them as model-checking problems and mechanizing the
requisite automata and $k$-induction machinery. \Cref{ch:multi-width} then
lifts this procedure from one symbolic width to many independent ones at once.
...
two of our tools solve 100\% of the deobfuscation dataset, and up to
\InstCombineNormCircuitVerifiedNumSolved{} of \InstCombineTotalProblems{}
(about 31\%) peephole rewrites extracted from LLVM's test suite.
...
\begin{figure}[htb]
\begin{align*}
\mathsf{A} &\equiv \mathsf{Const} ~|~ \mathsf{Var} \\
\mathsf{B} &\equiv \mathsf{A} ~|~ \mathsf{B} ~\mathsf{\boxplus}~ \mathsf{B} ~|~
  \bitnot \mathsf{B}, \quad \left(\boxplus \in \{\bitor, \bitand\}\right) \\
\mathsf{T} &\equiv \mathsf{B} ~|~ \mathsf{T} ~\mathsf{\otimes}~ \mathsf{T},
  \quad \left(\otimes \in \{+, - \}\right) \\
\mathsf{P} &\equiv (\mathsf{T} = 0).
\end{align*}
...
\label{sec:mono:conclusion}

For the mono-width setting ($\text{PBV}_1$), we fill the gap in ITP proof
automation for width-independent bitvector reasoning through verified decision
procedures: a $k$-induction-based model checker, an automata-theoretic solver,
and an MBA-Blast extension handling mixed arithmetic-bitwise predicates. These
tools solve 100\% of Hacker's Delight benchmarks and up to
\InstCombineNormCircuitVerifiedNumSolved{} of \InstCombineTotalProblems{}
(about 31\%) of LLVM peephole
rewrites, with the MBA solver achieving \MBAMBAGeoMean{} geomean runtime on
\MBATotalProblems{} problems. All are mechanized in Lean, producing fully
kernel-checked proofs via verified automata construction and $k$-induction.
...
\url{https://doi.org/10.5281/zenodo.16269885}~\cite{bhat_2025_16269885}; the
artifact has been independently reproduced through published artifact evaluation.
