...
We define the safety property $\Safe_k$ as the conjunction of two properties (\autoref{fig:swbv:decision-procedure-pesudocode}):
...
(\autoref{fig:swbv:k-induction-flow}).
%
...
def Predicate.denote (p : Predicate) (w : Nat) (vars : List (BitVec w)) : Prop :=
  ...
  | ... (other cases elided)
\end{lean}
The reflected predicate language is then transformed into the infinite stream semantics (\autoref{fig:swbv:stx-sem-bv-theory}),
...
where boolean terms are terms comprising of purely bitwise operations on atoms (\autoref{fig:swbv:mba-syntax}).
...
\noindent \paragraph{MBA~(\MBATotalProblems{} problems).} Out of the \MBATotalProblems{} problems (\autoref{fig:swbv:mba-cactus}), the k-induction solver solves \MBAKInductionVerifiedNumSolved{} problems out of \MBATotalProblems{},
...
\paragraph{InstCombine (\InstCombineTotalProblems{} problems).} In the \InstCombineTotalProblems{} problems (\autoref{fig:swbv:instcombine-cactus}),
