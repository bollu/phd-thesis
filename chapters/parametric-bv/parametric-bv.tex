...
\paragraph*{Attribution.} This section is based on \emph{Certified Decision
...
Tobias Grosser.
\emph{My contributions:} the initial algorithm---handling a single width and
...
\paragraph*{Attribution.} This section is based on \emph{Sound and Complete
...
\emph{My contributions:} I generalized the procedure from one symbolic width to
many, and recognized that encoding symbolic widths as bitmasks yields an
...
nonlinear fragment requires a genuinely different tool, which we sketch, via a
reduction to the first-order theory of the $2$-adic integers, in
\cref{sec:fw:2adic}. The second concerns evaluation. We deliberately avoid
...
correctness argument for the generalization. The single-width results, by
contrast, are packaged as an artifact that has been independently reproduced
through published artifact evaluation.
...
evaluation of \cref{sec:pbv:multi-width} is packaged into a Docker container as
the artifact for the multi-width results~\citep{bhat2026multiwidth}.
