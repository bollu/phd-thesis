...
def compare_tools_same_bw(df, bm, tool1, tool2):
    ...
    ax.legend(loc="center right", ncols=1, frameon=False, bbox_to_anchor=(1.2, 0.5))
    ax.set_yscale("log")
    save(fig, plotsdir + "_bv_decide_bitwuzla_" + bm.split(".")[0] + ".pdf")
...
def cumul_solving_time_smtlib(df, name):
    ...
    if name == 'unsat':
        # remove nan values from result column 
        result_bw_no_nan = df['result_bw'].dropna()
        result_lw_no_nan = df['result_lw'].dropna()
        result_lw_no_nan = [x for x in result_lw_no_nan if x != 'none']
        result_lwt_no_nan = df['result_lwt'].dropna()
        result_lwt_no_nan = [x for x in result_lwt_no_nan if x != 'none']
        result_coq_no_nan = df['result_coq'].dropna()
        bw_solved = len(result_bw_no_nan)
        lw_solved = len(result_lw_no_nan)
        lwt_solved = len(result_lwt_no_nan)
        coq_solved = len(result_coq_no_nan)
        # save these perf numbers to a file 
        file = open("smtlib_unsat_solver_summary_grant.txt", "w")
        file.write("bitwuzla solved problems: " + str(bw_solved) + "\n")
        file.write("bv_decide (+ kernel) solved problems: " + str(lw_solved) + "\n")
        file.write("bv_decide (no kernel) solved problems: " + str(lwt_solved) + "\n")
        file.write("CoqQFBV solved problems: " + str(coq_solved) + "\n")
        file.close()
