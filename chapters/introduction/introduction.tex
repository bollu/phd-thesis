...
\emph{given a peephole rewrite, how do I prove it correct at every width, with no SMT solver in the trusted base?}
...
\begin{enumerate}
\item Subtle algorithms can be fast, but they are hard to verify, and hard to change when the solver is updated.
\item Simple algorithms are easy to verify, but they can be far too slow to be useful.
\end{enumerate}
...
and in UNSAT certificates, where the LRAT format~\citep{lrat} was developed to
...
width-generic bitvector solvers that prove more problems than the unverified state of the art,
and the first formally verified floating-point bitblaster,
which is slower than Bitwuzla and offers in exchange guarantees that reach down to Lean's kernel.
...
and each of these chapters opens with an attribution note that states which parts of the work are mine.
